...
static void *mp_toom_cook_5_mul_threaded(void *p)
{
   ...
   struct toom_mul_5_param *params = p;

   B = (MAX(params->a->used, params->b->used)) / 5;

   sign = (params->a->sign != params->b->sign) ? MP_NEG : MP_ZPOS;

   if (MIN(params->a->used, params->b->used) < TOOM_COOK_5_MUL_CO) {
   ...
   for (count = 0; count < params->a->used; count++) {
      ...
      case 3:
         ...
         break;
      case 4:
         a4.dp[count - 4 * B] = params->a->dp[count];
         a4.used++;
         break;
      default:
         a4.dp[count - 4 * B] = params->a->dp[count];
   ...
   for (count = 0; count < params->b->used; count++) {
      ...
      case 3:
         ...
         break;
      case 4:
         b4.dp[count - 4 * B] = params->b->dp[count];
         b4.used++;
         break;
      default:
         b4.dp[count - 4 * B] = params->b->dp[count];
   ...
   struct toom_mul_5_param tparam_one = { .a = &a4, .b = &b4, .c = &w1, .depth = params->depth + 1 };
   struct toom_mul_5_param tparam_two = { .a = &a0, .b = &b0, .c = &w9, .depth = params->depth + 1 };

   if(params->depth < LMP_NUM_THREADS){
      pthread_t thread_one, thread_two;
// S1 = a4*b4
      pthread_create(&thread_one, NULL, mp_toom_cook_5_mul_threaded, &tparam_one);
// S9 = a0*b0
      pthread_create(&thread_two, NULL, mp_toom_cook_5_mul_threaded, &tparam_two);

      pthread_join(thread_one, NULL);
      pthread_join(thread_two, NULL);
   } else {
      mp_toom_cook_5_mul_threaded(&tparam_one);
      mp_toom_cook_5_mul_threaded(&tparam_two);
   }
...
int mp_toom_cook_5_mul(mp_int *a, mp_int *b, mp_int *c){
    ...
    struct toom_mul_5_param tparam = { .a = a, .b = b, .c = c , .depth = 0};
    mp_toom_cook_5_mul_threaded(&tparam);
    mp_exch(tparam.c,c);
    return tparam.err;
}
...
#include <omp.h>
   ...
   #pragma omp task shared(a4,b4,w1)
   e = mp_mul(&a4, &b4, &w1);
   if (e != MP_OKAY) goto ERR;
   #pragma omp task shared(a0,b0,w9)
   e = mp_mul(&a0, &b0, &w9);
   if (e != MP_OKAY) goto ERR;
   #pragma omp taskwait
#else
