...
void *mp_toom_cook_5_sqr_threaded(void *p)
{
   ...
   int e = MP_OKAY;
   int B, count, j;
   struct toom_sqr_5_param *params = p;

   B = (params->a->used) / 4;

   if (params->a->used < TOOM_COOK_5_SQR_CO) {
...
// A = a4*x^4+ a3*x^3 + a2*x^2 + a1*x + a0
   ...
   a3.used = B;
   for (j = 0; count < params->a->used; count++, j++) {
      a4.dp[j] = params->a->dp[count];
   }
   a4.used = j;
   mp_clamp(&a0);
   mp_clamp(&a1);
   ...
   struct toom_sqr_5_param tparam_one = { .a = &a0, .c = &w1, .depth = params->depth + 1 };
   struct toom_sqr_5_param tparam_two = { .a = &a4, .c = &w2, .depth = params->depth + 1 };

   if(params->depth < LMP_NUM_THREADS){
      pthread_t thread_one, thread_two;
// S1 = a0^2
      pthread_create(&thread_one, NULL, mp_toom_cook_5_sqr_threaded, &tparam_one);
// S2 = a4^2
      pthread_create(&thread_two, NULL, mp_toom_cook_5_sqr_threaded, &tparam_two);

      pthread_join(thread_one, NULL);
      pthread_join(thread_two, NULL);
   } else {
      mp_toom_cook_5_sqr_threaded(&tparam_one);
      mp_toom_cook_5_sqr_threaded(&tparam_two);
   }
...
// P = S2*x^8+ S9*x^7+ S4*x^6+ S3*x^5+ S6*x^4+ S5*x^3+ S7*x^2+ S8*x+ S1
   ...
   if ((e = mp_add(&tmp1, &w2, params->c)) != MP_OKAY) {
      goto ERR;
   }
ERR:
...
int mp_toom_cook_5_sqr(mp_int *a, mp_int *c){
    ...
    struct toom_sqr_5_param tparam = { .a = a, .c = c , .depth = 0};
    mp_toom_cook_5_sqr_threaded(&tparam);
    mp_exch(tparam.c,c);
    return tparam.err;
}
...
#include <omp.h>
   ...
   #pragma omp task shared(a0,w1)
   e = mp_sqr(&a0, &w1);
   if (e != MP_OKAY) goto ERR;
   #pragma omp task shared(a4,w2)
   e = mp_sqr(&a4, &w2);
   if (e != MP_OKAY) goto ERR;
   #pragma omp taskwait
#else
