...
Despite its large runtime it was the only way to include a deterministic primality test with a small memory footprint, no need for floating point functions and one that works with low MP_xBIT, too.
...
Not much work is needed but both sieves, the base sieve and the segment need to be cleared at the end.
...
int main(int argc, char **argv)
{
   ...
   for (k = 0, ret = 0; ret < number; k = ret) {
      if ((e = mp_next_small_prime(k + 1, &ret,
                                   &base, &segment,
                                   &single_segment_a)) != MP_OKAY) {
         ...
         fprintf(stderr,"mp_next_small_radix failed: \textbackslash{}"%s\textbackslash{}"\textbackslash{}n",
                 mp_error_to_string(e));
         goto LTM_ERR;
      }
...
This function free's all memory used. It does not zero out the used memory before free'ing it, use \texttt{mp\_factors\_zero} to clean the memory before free'ing it.
\item
\index{mp\_factors\_zero}
\verb!int mp_factors_zero(mp_factors *f);!\\
Zero the elements of the factor list and allocate (fresh) memory of default size in that order.
\item
...
Multiplies all elements of the list. It does not recognize sparse lists, every zero in the list gets multiplied, too. It does multiply the list with a binary-splitting algorithm which assumes a highly or better fully sorted list to work properly.
\end{description}
...
The function \texttt{mp\_factors\_product} does already most of the work so it was not much work to implement a function to compute a primorial.
\index{mp\_primorial}
\begin{alltt}
